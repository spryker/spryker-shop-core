...
runCommand() {
    ...
    if [[ -d "$CURRENT_PATH/Bundles/$module/src/" ]]; then
        SRC_FOLDER="$CURRENT_PATH/Bundles/$module/src/"
    fi

    if [ -d "$CURRENT_PATH/Bundles/$module/tests/" ]; then
        TESTS_FOLDER="$CURRENT_PATH/Bundles/$module/tests/"
    fi

    php -d memory_limit=-1 vendor/bin/phpcs $CURRENT_PATH/Bundles/$module/ --standard=$RULESET -p $SRC_FOLDER $TESTS_FOLDER
}
...
validateModuleCodeSniffer() {
  ...
  for module in $MODULES
      do
          ...
          local RULESET="$CURRENT_PATH/vendor/spryker/spryker/Bundles/Development/rulesetStrict.xml"
          if [ $LEVEL == 1 ]; then
              RULESET="$CURRENT_PATH/vendor/spryker/spryker/Bundles/Development/ruleset.xml"
          fi
...
validateModuleCodeSniffer SprykerShop
